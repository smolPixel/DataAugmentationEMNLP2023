...
class VAE_Link():
    ...
    def train_models(self):
        ...
        num_iter=min([len(dl) for dl in data_loaders])
        ...
        for i in range(self.argdict['nb_epoch_algo']):
            ...
            for it in range(num_iter):
                for cc, batches in enumerate(zip(data_loaders)):
                    for bb in batches:
                        batch_class=next(bb)
                        nll_batch, KL_batch, weight_batch= self.vae_per_class[cc].run_batches(batch_class, iteration, data_loaders[cc])
    ...
    def augment(self, train, dev):
        ...
        # train.return_pandas().to_csv(f'VAELink_train_{self.argdict["dataset"]}.tsv', sep='\t')
        # fds
        return train
