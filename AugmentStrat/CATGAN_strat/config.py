...
dataset = 'SST-2'  # oracle, image_coco, emnlp_news, amazon_app_book, amazon_app_movie, mr15
...
ADV_train_epoch = 1#2000  # SeqGAN, LeakGAN-200, RelGAN-3000
...
MLE_train_epoch = 150#150  # SeqGAN-80, LeakGAN-8, RelGAN-150
PRE_clas_epoch = 10
...
train_data = 'AugmentStrat/CATGAN_strat/dataset/' + dataset + '.txt'
test_data = 'AugmentStrat/CATGAN_strat/dataset/' + dataset + '_test.txt'
cat_train_data = 'AugmentStrat/CATGAN_strat/dataset/' + dataset + '_cat{}.txt'
cat_test_data = 'AugmentStrat/CATGAN_strat/dataset/' + dataset + '_cat{}_test.txt'
...
log_filename = strftime("AugmentStrat/CATGAN_strat/log/log_%s" % log_time_str)
...
save_root = 'AugmentStrat/CATGAN_strat/save/{}/{}/{}_{}_dt-{}_lt-{}_mt-{}_et-{}_sl{}_temp{}_lfd{}_T{}/'.format(time.strftime("%Y%m%d"),
...
oracle_state_dict_path = 'AugmentStrat/CATGAN_strat/pretrain/oracle_data/oracle_lstm.pt'
oracle_samples_path = 'AugmentStrat/CATGAN_strat/pretrain/oracle_data/oracle_lstm_samples_{}.pt'
multi_oracle_state_dict_path = 'AugmentStrat/CATGAN_strat/pretrain/oracle_data/oracle{}_lstm.pt'
multi_oracle_samples_path = 'AugmentStrat/CATGAN_strat/pretrain/oracle_data/oracle{}_lstm_samples_{}.pt'

pretrain_root = 'AugmentStrat/CATGAN_strat/pretrain/{}/'.format(dataset if if_real_data else 'oracle_data')
...
pretrained_clas_path = pretrain_root + 'clas_pretrain_{}_{}_sl{}_sn{}.pt'.format(run_model, model_type, max_seq_len,
                                                                                 samples_num)
signal_file = 'AugmentStrat/CATGAN_strat/run_signal.txt'
...
def init_param(opt, path):
    ...
    # Save path
    save_preroot='{}/CatGAN/save/{}/'.format(path, time.strftime("%Y%m%d"))
    save_root = '{}/CatGAN/save/{}/{}/{}_{}_dt-{}_lt-{}_mt-{}_et-{}_sl{}_temp{}_lfd{}_T{}/'.format(path, time.strftime("%Y%m%d"),
                                                                                         dataset, run_model, model_type,
                                                                                         d_type,
                                                                                         loss_type,
                                                                                         '+'.join(
    ...
    train_data = path+ '/CatGAN/dataset/' + dataset + '.txt'
    test_data = path+ '/CatGAN/dataset/' + dataset + '_test.txt'
    cat_train_data = path+ '/CatGAN/dataset/' + dataset + '_cat{}.txt'
    cat_test_data = path+ '/CatGAN/dataset/' + dataset + '_cat{}_test.txt'

    if max_seq_len == 40:
        oracle_samples_path = path+'/CatGAN/pretrain/oracle_data/oracle_lstm_samples_{}_sl40.pt'
        multi_oracle_samples_path = path+'/CatGAN/pretrain/oracle_data/oracle{}_lstm_samples_{}_sl40.pt'

    pretrain_root = '{}/CatGAN/pretrain/{}/'.format(path, dataset if if_real_data else 'oracle_data')
    ...
    dir_list = ['save', 'savefig', 'log', 'pretrain', 'dataset',
                '{}/CatGAN/pretrain/{}'.format(path, dataset if if_real_data else 'oracle_data')]
    if not if_test:
