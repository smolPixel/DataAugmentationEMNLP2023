...
from AugmentStrat.CATGAN_strat.metrics.bleu import BLEU
from AugmentStrat.CATGAN_strat.metrics.clas_acc import ACC
from AugmentStrat.CATGAN_strat.metrics.nll import NLL
from AugmentStrat.CATGAN_strat.metrics.ppl import PPL
from AugmentStrat.CATGAN_strat.utils.cat_data_loader import CatClasDataIter
from AugmentStrat.CATGAN_strat.utils.data_loader import GenDataIter
from AugmentStrat.CATGAN_strat.utils.helpers import Signal, create_logger, get_fixed_temperature
from AugmentStrat.CATGAN_strat.utils.text_process import load_dict, write_tokens, tensor_to_tokens
...
class BasicInstructor:
    def __init__(self, opt):
        ...
        # load dictionary
        self.word2idx_dict, self.idx2word_dict = load_dict(cfg.dataset, opt.path)

        # Dataloader
        # print(cfg.train_data)
        # self.train_data = GenDataIter(cfg.train_data, opt.path)
        self.train_data = GenDataIter(cfg.train_data, opt.path)
        self.test_data = GenDataIter(cfg.test_data, opt.path, if_test_data=True)

        self.train_data_list = [GenDataIter(cfg.cat_train_data.format(i), opt.path) for i in range(cfg.k_label)]
        self.test_data_list = [GenDataIter(cfg.cat_test_data.format(i), opt.path, if_test_data=True) for i in
                               range(cfg.k_label)]
        self.clas_data_list = [GenDataIter(cfg.cat_test_data.format(str(i)), opt.path, if_test_data=True) for i in
                               range(cfg.k_label)]
    ...
    def cal_metrics(self, fmt_str=False):
        ...
        with torch.no_grad():
            ...
            gen_data = GenDataIter(eval_samples, self.opt.path)
    ...
    def _save(self, phase, epoch):
        ...
        #Modifying the path so that it saves where I want it to
        write_tokens(f"{self.opt.path}/GeneratedData/GAN/samplesTemp.txt", tensor_to_tokens(samples, self.idx2word_dict))

    def _sample(self, num):
        samples = self.gen.sample(num)
        # write_tokens(save_sample_path, tensor_to_tokens(samples, self.idx2word_dict))
        # Modifying the path so that it saves where I want it to
        write_tokens(f"TEST_samples.txt",
