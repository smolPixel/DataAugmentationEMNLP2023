...
# ============================================
def init_dict(dataset, path):
    """
    Initialize dictionaries of dataset, please note that '0': padding_idx, '1': start_letter.
    Finally save dictionary files locally.
    """
    tokens = get_tokenlized('AugmentStrat/CATGAN_strat/dataset/{}.txt'.format(dataset))
    word_set = get_word_list(tokens)
    word2idx_dict, idx2word_dict = get_dict(word_set)
    with open('AugmentStrat/CATGAN_strat/dataset/{}_wi_dict.txt'.format(dataset), 'w') as dictout:
        dictout.write(str(word2idx_dict))
    with open('AugmentStrat/CATGAN_strat/dataset/{}_iw_dict.txt'.format(dataset), 'w') as dictout:
        dictout.write(str(idx2word_dict))

    # print('total tokens: ', len(word2idx_dict))


def load_dict(dataset, path):
    """Load dictionary from local files"""
    iw_path = 'AugmentStrat/CATGAN_strat/dataset/{}_iw_dict.txt'.format(dataset)
    wi_path = 'AugmentStrat/CATGAN_strat/dataset/{}_wi_dict.txt'.format(dataset)

    # if not os.path.exists(iw_path) or not os.path.exists(iw_path):  # initialize dictionaries
    init_dict(dataset, path)
...
def load_test_dict(dataset, path):
    """Build test data dictionary, extend from train data. For the classifier."""
    word2idx_dict, idx2word_dict = load_dict(dataset, path)  # train dict
    # tokens = get_tokenlized('dataset/testdata/{}_clas_test.txt'.format(dataset))
    tokens = get_tokenlized('AugmentStrat/CATGAN_strat/dataset/{}_test.txt'.format(dataset))
    word_set = get_word_list(tokens)
...
def process_cat_text():
    ...
    f_pos_test = open('dataset/{}{}_cat1_test.txt'.format(dataset, seq_len), 'w')
    f_neg_test = open('dataset/{}{}_cat0_test.txt'.format(dataset, seq_len), 'w')
    ...
    with open('dataset/{}{}_test.txt'.format(dataset, seq_len), 'w') as fout:
...
def extend_clas_train_data():
    ...
    neg_test_file = 'dataset/{}_cat0_test.txt'.format(dataset)
    pos_test_file = 'dataset/{}_cat1_test.txt'.format(dataset)
    ...
    with open('dataset/{}_cat0_clas_test.txt'.format(dataset), 'w') as fout:
        for line in neg_test:
            fout.write(line)
    with open('dataset/{}_cat1_clas_test.txt'.format(dataset), 'w') as fout:
        for line in pos_test:
            fout.write(line)
    with open('dataset/{}_clas_test.txt'.format(dataset), 'w') as fout:
        for line in neg_test:
            fout.write(line)
        for line in pos_test:
            fout.write(line)
