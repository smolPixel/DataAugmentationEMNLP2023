...
from AugmentStrat.CATGAN_strat.instructor import BasicInstructor
from AugmentStrat.CATGAN_strat.metrics.nll import NLL
from AugmentStrat.CATGAN_strat.modelsGAN.CatGAN_D import CatGAN_D, CatGAN_C
from AugmentStrat.CATGAN_strat.modelsGAN.CatGAN_G import CatGAN_G
from AugmentStrat.CATGAN_strat.utils.cat_data_loader import CatGenDataIter
from AugmentStrat.CATGAN_strat.utils.data_loader import GenDataIter
from AugmentStrat.CATGAN_strat.utils.gan_loss import GANLoss
from AugmentStrat.CATGAN_strat.utils.helpers import get_fixed_temperature
from AugmentStrat.CATGAN_strat.utils.text_process import tensor_to_tokens, write_tokens
...
class CatGANInstructor(BasicInstructor):
    ...
    def __init__(self, opt):
        ...
        # DataLoader
        self.all_train_data = CatGenDataIter(self.train_samples_list, opt.path)
    ...
    def variation(self, g_step, criterionG):
        ...
        for step in range(g_step):
            ...
            for i, (d_out_real, fake_samples) in enumerate(zip(self.d_out_real, dis_gen_samples)):  # share real
                # d_out_real = self.dis(real_samples)
                d_out_fake = self.dis(fake_samples)
                # print(d_out_fake.shape, d_out_real.shape)
    ...
    def evaluation(self, eval_type):
        ...
        if cfg.lambda_fd != 0:
            nll_div = []
            for label_i in range(cfg.k_label):
                gen_data = GenDataIter(eval_samples[label_i], self.opt.path)
    ...
    def _sample(self, num, label_i):
        assert type(label_i) == int
        samples = self.gen.sample(num, self.opt.batch_size, label_i=label_i)
        return tensor_to_tokens(samples, self.idx2word_dict)
        # write_tokens(f"{self.opt.path}/GeneratedData/CATGAN/samples_{label_i}_{k}.txt", tensor_to_tokens(samples, self.idx2word_dict))
    ...
    def prepare_train_data(self, which, step=None):
        ...
        real_samples_list = [
            F.one_hot(self.train_data_list[i].random_batch()['target'][:cfg.batch_size],
    ...
    def prepare_eval_real_data(self):
        ...
        with torch.no_grad():
            self.eval_real_samples = [torch.cat(
                [F.one_hot(self.train_data_list[i].random_batch()['target'], cfg.vocab_size).float()
