...
class EDA():
    ...
    def augment(self, train, dev, return_dict=False):
        ...
        sentAug=eda(sentence, self.argdict['alpha_sr'], self.argdict['alpha_ri'], self.argdict['alpha_rs'], self.argdict['alpha_rd'], 1, language=train.language)[0]
        diconew={}
        ...
        for i in range(int(self.argdict['split'])):
            for j, ex in train.iterexamples():
                ...
                sentAug=eda(line, self.argdict['alpha_sr'], self.argdict['alpha_ri'], self.argdict['alpha_rs'], self.argdict['alpha_rd'], 1, language=train.language)[0]
        ...
        # train.return_pandas().to_csv(f'EDASD_train_{self.argdict["dataset"]}.tsv', sep='\t')
        # fds
        return train
