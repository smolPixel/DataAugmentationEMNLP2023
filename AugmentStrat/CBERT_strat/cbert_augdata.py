...
def main():
    ...
    parser.add_argument('--temp', default=1.0, type=float,
                        help="temperature")
    parser.add_argument('--num_aug', default=1.0, type=float, help='number of time to perform the augmentation')
    ...
    with open("CBERT/global.config", 'r') as f:
    ...
    save_train_path = os.path.join(args.output_dir, "train_augment.tsv")
    ...
    for num_augmentation_to_do in range(int(args.num_aug)):
        print(f"Augmenting {num_augmentation_to_do} time")
        torch.cuda.empty_cache()
        cbert_name = "{}/BertForMaskedLM_{}_epoch_{}".format(task_name, task_name, 1)
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            for ids, idx, preds, seg in zip(init_ids, masked_idx, predictions, segment_ids):
                ...
                for pred in preds:
                    ...
                    # tsv_writer.writerow([new_str, 3])
            torch.cuda.empty_cache()
        predictions = predictions.detach().cpu()
        model.cpu()
        torch.cuda.empty_cache()
        bak_train_path = os.path.join(args.output_dir, "train_augment.tsv")
