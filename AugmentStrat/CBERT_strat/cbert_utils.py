...
class AugProcessor(DataProcessor):
    ...
    def get_labels(self, name):
        ...
        if name in ['stsa.binary', 'mpqa', 'rt-polarity', 'subj', "binaryData"]:
            return ["0", "1"]
        elif name in ['stsa.fine']:
            return ["0", "1", "2", "3", "4"]
        elif name in ["FourClassData"]:
            return ["0", "1", "2", "3"]
        elif name in ['TREC', 'SixClassData']:
            return ["0", "1", "2", "3", "4", "5"]
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, random_seed):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens, init_ids, input_ids, input_mask, segment_ids, masked_lm_labels = \
            extract_features(tokens_a, tokens_label, max_seq_length, tokenizer, random_seed)
...
def construct_train_dataloader(train_examples, label_list, max_seq_length, train_batch_size, num_train_epochs, tokenizer, device, random_seed):
    """construct dataloader for training data"""
    
    num_train_steps = None
    global_step = 0
    train_features = convert_examples_to_features(
        train_examples, label_list, max_seq_length, tokenizer, random_seed)
...
def extract_features(tokens_a, tokens_label, max_seq_length, tokenizer, random_seed):
