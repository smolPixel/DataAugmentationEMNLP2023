...
import AugmentStrat.CBERT_strat.cbert_utils as cbert_utils
...
def fine_tune_model(argdict):
    ...
    ## Required parameters
    data_dir="AugmentStrat/CBERT_strat/datasets"
    output_dir="aug_data"
    save_model_dir="cbert_model"
    bert_model="bert-base-uncased"
    task_name="binaryData"
    max_seq_length=64
    do_lower_case=True
    train_batch_size=32
    learning_rate=5e-5
    ...
    processors = {
        ...
        "SixClassData": AugProcessor,
        "FourClassData": AugProcessor,
        "binaryData": AugProcessor,
        "TREC": AugProcessor,
    ...
    elif task_name in ['FourClassData']:
        model.bert.embeddings.token_type_embeddings = torch.nn.Embedding(4, 768)
        model.bert.embeddings.token_type_embeddings.weight.data.normal_(mean=0.0, std=0.02)
    elif task_name in ['TREC', 'SixClassData']:
    ...
    train_features, num_train_steps, train_dataloader = cbert_utils.construct_train_dataloader(train_examples, label_list, max_seq_length,
                                               train_batch_size, num_train_epochs, tokenizer, device,
                                               seed)
    ...
    optimizer = AdamW(optimizer_grounded_parameters, lr=learning_rate, correct_bias=False)
    model.train()

    os.makedirs(save_model_dir, exist_ok=True)
    save_model_dir = os.path.join(save_model_dir, task_name)
...
#         args.train_batch_size, args.num_train_epochs, tokenizer, device, args.seed)
#
#     ## if you have a GPU, put everything on cuda
#     model.cuda()
#
