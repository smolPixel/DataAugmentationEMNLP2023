...
class CBERT():
    ...
    def init_model(self):
        ...
        for cat in [f"[{cat.lower()}]" for cat in self.argdict['categories']]:
            self.tokenizer.add_tokens(cat, special_tokens=True)
    ...
    def clean_sentence(self, sent):
        ...
        clean_sent=[]
        sent=sent.split(" ")
        # arr=[self.argdict['categories'][lab]]
        ll=['[CLS]']
        ll.extend([f"[{cat.lower()}]" for cat in self.argdict['categories']])
        # print(ll)
        ...
        ss=" ".join(clean_sent)
        #additional check to be sure the class is not in there
        for tok in ll:
            ss=ss.replace(tok, "")
        return ss
    ...
    def augment(self, train, dev, return_dict=False):
        ...
        for batch in data_loader:
            ...
            for i in range(int(self.argdict['split'])):
                with torch.no_grad():
                    ...
                    # batch['sentence']=['This is a good movie', 'This is a good movie']
                    # batch['label']=torch.Tensor([1,1]).int()
        ...
        # train.return_pandas().to_csv(f'CBERT_train_{self.argdict["dataset"]}.tsv', sep='\t')
        # fds
        return train
