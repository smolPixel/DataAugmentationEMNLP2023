"""VAE augmentation"""
import pandas
from AugmentStrat.PVAE_strat.PVAE import PVAE_meta
from data.DataProcessor import separate_per_class
import torch
from AugmentStrat.PVAE_strat.utils import to_var
import math
import itertools
import numpy as np
import random
...
class PVAE():
    ...
    def init_models(self, training_sets):
        for classe, training_set in enumerate(training_sets):
            vae=PVAE_meta(self.argdict, training_set)
    ...
    def train_models(self):
        ...
        for cc in range(num_cat):
            ...
            for i in range(self.argdict['nb_epoch_algo']):
                ...
                for batch in dl:
                    nll_batch, KL_batch, weight_batch= self.vae_per_class[cc].run_batches(batch, iteration, dl)
                    iteration+=1
