...
class Perfect():
    ...
    def __init__(self, argdict):
        ...
        self.num_categories=len(self.argdict['categories'])
    ...
    def augment(self, train, return_dict=False):
        ...
        num_per_class_added=[0]*self.num_categories
        ...
        for i, ex in train_complete.iterexamples():
            ...
            #     if self.argdict['corrupt_data'] and nb_corrupt_neg<nb_to_corrupt:
            #         ex['label']=1
            #         nb_corrupt_neg+=1
            #     # train[len(train)]=ex
            #     diconew[len(diconew)] = {'sentence': ex['sentence'],
            #                              'label': ex['label'],
            #                              'input': ex['input'],
            #                              'augmented': True}
            #     num_neg+=1
            # elif ex['label']==1 and num_pos<num_per_class_to_add:
            #     if self.argdict['corrupt_data'] and nb_corrupt_pos<nb_to_corrupt:
            #         ex['label']=0
            #         nb_corrupt_pos+=1
            #     # train[len(train)]=ex
            elif num_per_class_added[ex['label']]<num_per_class_to_add:
                diconew[len(diconew)] = {'sentence': ex['sentence'],
                                         'label': ex['label'],
                                         'input': ex['input'],
                                         'augmented': True}
                num_per_class_added[ex['label']]+=1
